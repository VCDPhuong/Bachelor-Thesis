...
\begin{frame}
\quad With a 2D electron gas in an external field field \(\textbf{B} = (0,0,B)\) \& \(\textbf{E} = (E,0,0)\), their energy will be collapsed to the Landau levels:
...
\end{equation*}
\quad The electron will drift in the \(O_y\) direction by:
\begin{equation}\label{eq: vy}
v_y = \frac{1}{\hbar} \pdv{E_{\nu,k_y}}{k_y} = -\frac{E}{B}
\end{equation}

Classical drift along: \(\textbf{E} \cross \textbf{B}\)
\end{column}
...
\end{frame}
\begin{frame}
\quad But, how we can got the round (not sharps) as in fig. \ref{Fig: QHE}?\\\vspace{0.05cm}
{\centering $\Rightarrow$ Turn out the disorder (impurity) play as another key role!}\\\vspace{0.05cm}
\begin{columns}
\begin{column}{0.6\linewidth}
Disorder causes:\null
\begin{itemize}
\item Perturbation \(\to\) Broader side (peaks!).\\
\item \semitransp{Catch the localized state \(\to\) Plateux!}
\end{itemize}
\vspace{0.2cm}
